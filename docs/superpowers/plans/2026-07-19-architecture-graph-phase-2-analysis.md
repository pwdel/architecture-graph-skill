...
- Produces: `validate_typed_record(record, expected_kind=None) -> tuple[ValidationIssue, ...]`; both non-deterministic derivation times and every human review `created_at` use `canonical_utc_event_time`.
...
Typed review validation is equally exhaustive. In addition to its target, verdict, replacement, authority, evidence, and supersession shape, every `review` requires `created_at` to be a non-null string accepted by `canonical_utc_event_time`. Null, offset-form, fractional, non-zero-padded, and invalid-calendar values produce a `ValidationIssue` at `created_at`; no ledger reader, reducer, frozen review set, or snapshot finalizer may treat them as valid persisted events.
...
`raw-proposals.jsonl` contains: same-identity modality replacement at `/qualifiers/modality`; identity-changing subject replacement at `/subject/entity_id`; and unreviewed alias proposal. Each proposal points to a finalized `producer_kind: llm` derivation built with fixed `created_at: 2026-07-19T10:00:00Z`; a fixture variant with null or offset-form time must fail typed validation. `raw-reviews.jsonl` contains: stale verification; lower-authority verify plus higher-authority dispute; equal-authority incompatible verdicts; accepted modality proposal; accepted subject proposal; rejected alias proposal; and explicit same-reviewer supersession. Every human review uses a fixed canonical `YYYY-MM-DDTHH:MM:SSZ` time. Test helpers parse every raw line and call `finalize_record`; hand-calculated digests do not enter fixtures. The test-only `finalized_review_with_created_at(value)` helper starts from one otherwise valid raw review payload, replaces `created_at`, and recomputes its review ID and content digest through the production identity/finalization helpers before calling typed validation. It does not bypass or duplicate the validator under test.
...
@pytest.mark.parametrize(
    ("created_at", "valid"),
    [
        ("2026-07-19T10:00:00Z", True),
        (None, False),
        ("2026-07-19T10:00:00+00:00", False),
        ("2026-07-19T10:00:00.000Z", False),
        ("2026-02-30T10:00:00Z", False),
    ],
)
def test_review_created_at_requires_canonical_utc_event_time(
    review_fixture, created_at: str | None, valid: bool
) -> None:
    review = review_fixture.finalized_review_with_created_at(created_at)
    issues = validate_typed_record(review, "review")
    if valid:
        assert issues == ()
    else:
        assert any(issue.field == "created_at" for issue in issues)
...
- [ ] **Step 2: Implement typed review-time validation, policy validation, and conservative review reduction**

Every production ledger path runs the `review` branch of `validate_typed_record` before policy or reduction; that branch calls the shared `canonical_utc_event_time` helper for the required `created_at`. `reduce_reviews` remains defensive when a unit/in-memory caller supplies a semantically invalid review: it retains the ID in `invalid_review_ids` and excludes the event, but it never normalizes an offset, truncates fractions, substitutes a time, or silently applies a malformed event. `AuthorityPolicy` canonicalizes reviewer authorities and hashes them. A review applies only when reviewer ID, copied authority, authority-policy digest, target, target content digest, supersession ancestry, verdict, target kind, field path, and canonical event time validate. Retain stale/invalid IDs for reporting but exclude them from effective reduction. A later event from the same reviewer supersedes an earlier one only through explicit `supersedes_review_id`. Reduce at highest authority; incompatible equal-authority verdicts become disputed.
...
- Produces: `AnalysisResult(provenance_layer, records_by_type, report)` and `snapshot_records() -> tuple[Record, ...]`; a result owns every record produced or carried by that analysis call, stable-flattens them for publication, and contains at most one content version per stable ID for one provenance layer.
- Produces interim `analyze_corpus_intrinsic(corpus: IngestedCorpus, runtime: NlpRuntime, project_config: ProjectConfig, configuration_digest: str, pipeline_digest: str, proposals=(), reviews=()) -> AnalysisResult` for the deterministic layer. This function calls `parse_corpus` internally and owns every parsed warning and derivation; it never returns `ParsedCorpus` to the indexer.
...
Add this production-orchestration ownership regression. `analysis_fixture.index_with_stage_trace()` invokes the real `index_repository`; test-only spies capture the actual `AnalysisResult`, `ParsedCorpus`, and `RelationCandidateResult` returned inside that call without parsing again or changing the production return type. Its runtime exposes tokenization, dependency/noun-chunk, and named-entity capabilities and one parser warning template.

```python
from collections import Counter


def test_analysis_owns_parsed_records_and_publishes_each_once(
    analysis_fixture,
) -> None:
    reader, analysis, parsed, relations = analysis_fixture.index_with_stage_trace()
    analysis_derivations = Counter(
        item["id"] for item in analysis.records_by_type["derivations"]
    )
    analysis_warnings = Counter(
        item["id"] for item in analysis.records_by_type["warnings"]
    )
    final_derivations = Counter(item["id"] for item in reader.iter("derivations"))
    final_warnings = Counter(item["id"] for item in reader.iter("warnings"))

    for item in parsed.derivations:
        assert analysis_derivations[item["id"]] == 1
        assert final_derivations[item["id"]] == 1
    for item in parsed.warnings:
        assert analysis_warnings[item["id"]] == 1
        assert final_warnings[item["id"]] == 1

    capability_ids = {
        item["id"] for item in parsed.derivation_by_capability.values()
    }
    referenced_capability_ids = {
        derivation_id
        for term in analysis.records_by_type["terms"]
        for derivation_id in term["derivation_ids"]
        if derivation_id in capability_ids
    } | {
        derivation_id
        for candidate in relations.candidates
        for derivation_id in candidate.derivation_ids
        if derivation_id in capability_ids
    }
    assert referenced_capability_ids
    assert all(
        final_derivations[derivation_id] == 1
        for derivation_id in referenced_capability_ids
    )
```
...
Add the persisted-ledger time regression below. `review_publication_fixture.replace_ledger_with_unvalidated_created_at` rewrites its one otherwise valid review through the raw JSONL fixture path, recomputes the matching review ID and content digest with the production identity/finalization helpers, and intentionally bypasses only typed validation so `created_at` is the record's sole defect and read-side defenses are exercised. `mutation_state()` captures exact ledger bytes, the sorted staging tree and file bytes, observation-log bytes (or absence), and `current.json` bytes (or absence). Both CLI helpers run the real entry points.
...
def test_persisted_noncanonical_review_blocks_append_and_finalize_without_mutation(
    review_publication_fixture, created_at: str | None
) -> None:
    fixture = review_publication_fixture.with_published_deterministic_parent()
    fixture.replace_ledger_with_unvalidated_created_at(created_at)
    before = fixture.mutation_state()

    assert fixture.cli_append_valid_review() == 2
    assert fixture.mutation_state() == before
    assert fixture.cli_finalize_reviews() == 2
    assert fixture.mutation_state() == before
```
...
`analyze_corpus_intrinsic` accepts the validated `ProjectConfig` and already loaded `NlpRuntime`, then calls in order: parse; terms; relation candidates; qualification of `claim_eligible_candidates`; entity resolution with the exact immutable `IngestedCorpus` plus `project_config.aliases`; claims; deterministic decisions; deterministic diagnostics; current/review/historical deterministic lifecycle projections; intrinsic features/retrieval; rankings; report from the required current/review selection map. Immediately after decision reduction it computes `final_claims = tuple(updated_claims_by_id.get(claim["id"], claim) for claim in original_claims)`, appends only explicitly new IDs, and asserts one content version per stable ID before building the deterministic catalog. The function is the sole owner of its `ParsedCorpus`: insert every item from `ParsedCorpus.warnings` into `records_by_type["warnings"]` and every stable-ID-deduplicated item from `ParsedCorpus.derivations` into `records_by_type["derivations"]` exactly once before validating term/relation upstream references. Add all other analysis-produced records under their canonical family, with canonical empty proposal/review families for the deterministic layer; Phase 1 ingestion records are not copied into this result. `snapshot_records()` is the only deterministic analysis-record handoff to the indexer. Deduplicate derivations/edges by stable ID and reject same-ID/different-digest collisions.
...
- Produces final `analyze_corpus(corpus: IngestedCorpus, runtime: NlpRuntime, project_config: ProjectConfig, history: DecisionHistory, configuration_digest: str, pipeline_digest: str, proposals=(), reviews=()) -> AnalysisResult`; prior decisions/source lineage are owned by `DecisionHistory` and are not separate arguments, and parsing remains internal to this analysis boundary.
